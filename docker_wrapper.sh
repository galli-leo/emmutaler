...
echo "Running: docker run --rm -p 1234:1234 -v $PARENT:/app -v $SYSROOT:/sysroot -e QEMU_LD_PREFIX=/sysroot $DEBUG_ARGS emmu-run $RELPATH $@"
# TODO automatic debugging
docker run -it --rm -v $PARENT:/app -v $SYSROOT:/sysroot -e QEMU_LD_PREFIX=/sysroot $DEBUG_ARGS emmu-run $RELPATH $@