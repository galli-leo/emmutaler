...
def post_process(text_start):
    log.info("Defining start")
    # idaapi.add_entry(0, 0, "start", 1)
    # ida_funcs.add_func(text_start)

    log.info("Finding functions")
    ...
    log.info("Identified %d new functions", nfunc)

    log.info("Looking for interesting functions")
    try:
        find_interesting(segment_start, segment_end)
    except:
        log.exception("Failed to find interesting!")