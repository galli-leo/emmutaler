...
var readASN1TestData = []readASN1Test{
	{"valid", []byte{0x30, 2, 1, 2}, asn1.FromLowNum(0x30), true, []byte{1, 2}},
	{"truncated", []byte{0x30, 3, 1, 2}, asn1.SEQUENCE, false, nil},
	{"zero length of length", []byte{0x30, 0x80}, asn1.SEQUENCE, false, nil},
	{"invalid long form length", []byte{0x30, 0x81, 1, 1}, asn1.SEQUENCE, false, nil},
	{"non-minimal length", append([]byte{0x30, 0x82, 0, 0x80}, make([]byte, 0x80)...), asn1.SEQUENCE, false, nil},
	{"invalid tag", []byte{0xa1, 3, 0x4, 1, 1}, 31, false, nil},
	{"high tag", []byte{0x1f, 0x81, 0x80, 0x01, 2, 1, 2}, 0x4001, true, []byte{1, 2}},
	{"high but could have been low tag", []byte{0x1f, 0x80, 2, 1, 2}, 0x4001, false, nil},
	{"high too long tag", []byte{0x1f, 0x81, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 2, 1, 2}, 0x4001, false, nil},
	{"high private tag", []byte{0xFF, 0x84, 0xEA, 0x85, 0x9C, 0x42, 2, 1, 2}, asn1.Tag(0x4d414e42).WithClass(asn1.ClassPrivate).Constructed(), true, []byte{1, 2}},
	{"2**31 - 1 length", []byte{0x30, 0x84, 0x7f, 0xff, 0xff, 0xff}, asn1.SEQUENCE, false, nil},
	{"2**32 - 1 length", []byte{0x30, 0x84, 0xff, 0xff, 0xff, 0xff}, asn1.SEQUENCE, false, nil},
	{"2**63 - 1 length", []byte{0x30, 0x88, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, asn1.SEQUENCE, false, nil},
	{"2**64 - 1 length", []byte{0x30, 0x88, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, asn1.SEQUENCE, false, nil},
}
...
var optionalOctetStringTestData = []struct {
	...
	{readASN1Test{"empty", []byte{}, asn1.FromLowNum(0xa0), true, []byte{}}, false},
	{readASN1Test{"invalid", []byte{0xa1, 3, 0x4, 2, 1}, asn1.FromLowNum(0xa1), false, []byte{}}, true},
	{readASN1Test{"missing", []byte{0xa1, 3, 0x4, 1, 1}, asn1.FromLowNum(0xa0), true, []byte{}}, false},
	{readASN1Test{"present", []byte{0xa1, 3, 0x4, 1, 1}, asn1.FromLowNum(0xa1), true, []byte{1}}, true},
}
...
func TestAddASN1(t *testing.T) {
	testData := []struct {
		in  asn1.Tag
		out []byte
	}{
		{asn1.SEQUENCE, []byte{0x30, 0}},
		{asn1.FromChar("BNCH").WithClass(asn1.ClassPrivate).Constructed(), []byte{0xFF, 0x84, 0x92, 0xB9, 0x86, 0x48, 0}},
		{asn1.BIT_STRING, []byte{0x3, 0}},
		{asn1.FromChar("MANP").WithClass(asn1.ClassPrivate).Constructed(), []byte{0xFF, 0x84, 0xEA, 0x85, 0x9C, 0x50, 0}},
	}

	for _, test := range testData {
		b := NewBuilder([]byte{})
		b.AddASN1(test.in, func(child *Builder) {
			//ja nei
		})
		res, err := b.Bytes()
		if err != nil {
		...
		if len(res) != len(test.out) {
			t.Fatalf("Mismatched result length, expected %v, got %v", test.out, res)
		}
		for i, bt := range res {
			if bt != test.out[i] {
				t.Errorf("Expected %v got %v as result", test.out, res)
			}
		}
	}
}
